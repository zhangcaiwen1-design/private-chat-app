...
async function ensureContact() {
  const seedName = `图片恢复联系人 ${Date.now().toString().slice(-6)}`;
  const seedPhone = `136${Date.now().toString().slice(-8)}`;
...
async function seedImageCloudBackup() {
  const contact = await ensureContact();

  const uniqueUri = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQIW2NkYGD4DwABBAEAffR0WQAAAABJRU5ErkJggg==';
  ...
  if (!response.ok) {
    throw new Error(`图片云记录创建失败: ${response.status}`);
  }

  return { uniqueUri, contactName: contact.name };
}

async function main() {
  const browser = await chromium.launch({ channel: 'msedge', headless: false });
  const page = await browser.newPage();
  const { uniqueUri, contactName } = await seedImageCloudBackup();
  ...
  await page.getByText('已保存图片', { exact: true }).first().click();
  await page.getByText('云端图片预览', { exact: true }).waitFor({ state: 'visible', timeout: 10000 });
  await page.locator(`img[src="${uniqueUri}"]`).first().waitFor({ state: 'visible', timeout: 10000 });
  ...
  await page.locator(`img[src="${uniqueUri}"]`).last().waitFor({ state: 'visible', timeout: 10000 });
  ...
  console.log(JSON.stringify({ uniqueUri, contactName, imagePreviewOpened: true, restoredToLocal: true, openedConversationDirectly: true, restoreDisabledAfterSuccess: true }));
  await browser.close();
}
