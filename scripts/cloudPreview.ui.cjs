...
async function seedCloudOnlyBackup() {
  const contact = await ensureContact();

  const uniqueText = `云恢复 ${Date.now()}`;
  ...
  if (!response.ok) {
    throw new Error(`创建云记录失败: ${response.status}`);
  }

  return { uniqueText, contactName: contact.name };
}

async function main() {
  const browser = await chromium.launch({ channel: 'msedge', headless: false });
  const page = await browser.newPage();
  const { uniqueText, contactName } = await seedCloudOnlyBackup();
  ...
  await page.locator('text=/\\d{2}-\\d{2} \\d{2}:\\d{2}/').first().waitFor({ state: 'visible', timeout: 10000 });
  await page.getByText(uniqueText, { exact: true }).click();
  await page.getByText('云端内容预览', { exact: true }).waitFor({ state: 'visible', timeout: 10000 });
  await page.getByText(uniqueText, { exact: true }).last().waitFor({ state: 'visible', timeout: 10000 });
  ...
  console.log(JSON.stringify({ uniqueText, contactName, previewOpened: true, restoredToLocal: true, openedConversationDirectly: true, restoreDisabledAfterSuccess: true, noSuccessDialog: true }));
  await browser.close();
}
